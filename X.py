...
class Player(sprite.Sprite):
    def __init__(self, health, stamina, mana, inventory, currentWeapon, currentArmour, currentBoots, money, kind, damage, defense, hotbar):
        ...
        self.fogMaps = [transform.scale(image.load('Art\Misc\Fog.png'),(900,600)).convert(),transform.scale(image.load('Art\Misc\Fog.png'),(900,600)).convert(),transform.scale(image.load('Art\Misc\Fog.png'),(900,600)).convert(),transform.scale(image.load('Art\Misc\Fog.png'),(900,600)).convert()] 
        self.mapFog = self.fogMaps[0]
    ...
    def npcInteract(self,currentQuest):#IAN
       # if kb[K_p]:#LOCATION, remove later
        #    print(-back_x+530,-back_y+345)
        ...
        if armorShop.collidepoint(-back_x+530,-back_y+345):#IAN
            ...
            if self.kind == 'Charger':
                ...
                if dist<100:
                    if randint(1,100) == 1 and len(enemies)<150:
                        enemies.append(Enemy(100,choice(['Charger','Archer','Ghost','Dragon','Mage']),self.x+choice([-20,20]),self.y+choice([-20,20])))
                else:
                    self.vx -= int(self.speed*cos(self.angle))
                    self.vy -= int(self.speed*sin(self.angle))
...
class Boss(sprite.Sprite):
    ...
    def createHealTotem(self): #Creates a totem for the enemy to heal in a random corner of the map
        self.healRect = Rect(choice([100,924]),choice([100,600]),50,50)
        self.healing = True
...
def drawShopItems(shopItems,mx,my,pic):
    ...
    for item in shopItems:
        ...
        if shopRects[shopItems(item)].collidepoint(mx,my):
...
def characterSelection(click,choiceRects,backRect):
    ...
    if backRect.collidepoint(mx,my) and click:
        mode = 0
        titleMode = 0
    return mode,titleMode,player,mapNum
...
def drawIntroScreen(tx,ty,titleScrollx,titleScrolly,back,back_x,back_y,textbox1,name,fire,firePos,titleMenu,titleRects,titleMode,click,choiceRects,backRect,mx,my,running):
    ...
    if titleMode == 0:
        ...
        mode, titleMode, player,mapNum = characterSelection(click,choiceRects,backRect)
    elif titleMode == 2:
        mode = 0
        running = False
    return mode,0,0,firePos,running,titleMode,player,tx,ty,titleScrollx,titleScrolly,mapNum
...
def changeMap(mapList,mapNum,back_mask,back_x,back_y,back,player):
    if back_mask.get_at((int(-back_x+player.x),int(-back_y+player.y))) == (255,255,0):
        if mapNum == 0:
            if int(-back_x+player.x)<back.get_width()/4:
                return 1,-2100,-1000,900,400 #mapNum,back_x,back_y,player.x,player.y
            elif int(-back_x+player.x)>3*back.get_width()/4: #Change these for right map
                return 3,1,1,1,1 #These arent right just temp
            elif int(-back_y+player.y)>back.get_height()/4:
                return 2,1,1,1,1 #Not right, just temp
        elif mapNum == 1:
            return 0,0,-1000,90,250
        elif mapNum == 2:
            print('hi')#Temp (follow elif mapNum == 1 statement for how to write this
        elif mapNum == 3:
            print('hi')#temp
    else:
        return mapNum,back_x,back_y,player.x,player.y
#===========================================================================================================================================#
...
helpMeRects=[Rect(1040,505,50,50),Rect(1740,570,50,50),Rect(1740,1160,50,50),Rect(2160,1970,50,50)
             ,Rect(2255,1790,50,50)]
...
questSpeech=[image.load("Art//Quests//quest1Speech.png"),image.load("Art//Quests//quest2Speech.png"),image.load("Art//Quests//quest3Speech.png"),image.load("Art//Quests//quest2Speech.png")
...
#====================================================================================================================================================================#
    ...
    #============= Updates ============#
        ...
        draw.rect(back,(255,0,0),currentQuest[0])#IAN Remove later
        back = mapList[mapNum][0]
        back_mask = mapList[mapNum][1]
        mapNum,back_x,back_y,player.x,player.y = changeMap(mapList,mapNum,back_mask,back_x,back_y,back,player)
    #==================================#
    click = False
    rclick = False
    display.flip()
