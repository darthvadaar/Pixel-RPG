...
class Enemy(sprite.Sprite):
    ...
    def AI(self,back_x,back_y,back_mask):
        ...
        if self.kind == 'Charger' and -10<=self.rect[0]<=1034 and -10<=self.rect[1]<=745 and wallCol(back_mask,[self.rect.center],back_x,back_y) == [False]:
            self.x += int(enemy.speed*cos(self.angle))
            self.y += int(enemy.speed*sin(self.angle))
        elif self.kind == 'Archer' and 30<=self.rect[0]<=900 and 30<=self.rect[1]<=750 and wallCol(back_mask,[self.rect.center],back_x,back_y) == [False]:
